...
const ROUNDS = [
  ...
  { id: "indian",    label: "Bollywood in English" },
...
let specCorrectCountUnsub = null;
...
const playersMap = {};
// cache spectator stats (uid -> { name, correctCount })
const spectatorStats = new Map();
...
/* ---------- Referee Logic ---------- */
async function pickAndSetReferee(roomCode) {
  const roomSnap = await getDoc(roomDoc(roomCode));
  const roomData = roomSnap.exists() ? (roomSnap.data() || {}) : {};
  let used = Array.isArray(roomData.refereeUsed) ? roomData.refereeUsed : [];

  const playersSnap = await getDocs(playersCol(roomCode));
  const allPlayers = playersSnap.docs.map(d => d.id);

  // Remove refs no longer in room
  used = used.filter(uid => allPlayers.includes(uid));

  // Determine pool = players who have NOT been referee yet this cycle
  const pool = allPlayers.filter(uid => !used.includes(uid));

  // If everyone has been referee â†’ reset for new cycle
  const actualPool = pool.length > 0 ? pool : allPlayers;

  // Choose one from actual pool
  const chosen = actualPool[Math.floor(Math.random() * actualPool.length)];

  // Update used list
  const newUsed = used.includes(chosen) ? used : [...used, chosen];

  const chosenPlayerDoc = playersSnap.docs.find(d => d.id === chosen)?.data() || {};
  const chosenName = chosenPlayerDoc.name || "Player";

  await updateDoc(roomDoc(roomCode), {
    refereeUid: chosen,
    refereeUsed: newUsed,
    roundBanner: `Referee: ${chosenName}`,
    roundBannerAt: serverTimestamp(),
    action: `Referee assigned: ${chosenName}`
  });

  return chosen;
}

async function enableRefereeRotation(roomCode) {
  await updateDoc(roomDoc(roomCode), {
    refereeAuto: true,
    refereeUsed: [] // start fresh cycle
  });

  await pickAndSetReferee(roomCode);
}
...
async function adjustScore(targetUid, delta = 10) {
  ...
  try {
    ...
    showActionToast("Score update failed â€” check console");
  }
}

/* -------------------- buzzer: one buzz per frame + UI auto-disable -------------------- */
async function pressBuzzerOnce(roomCode, frameIndex, roundIndex, uid, playerName){
  ...
  await runTransaction(db, async (tx) => {
    ...
    tx.set(key, {
      ...
      ts: serverTimestamp(),
      uid,
      roundIndex: Number(roundIndex ?? 0)   // ðŸ‘ˆ store the round to avoid cross-round bleed
    });                                                                   
  });
}
...
function watchMyBuzzDoc(roomCode, frameIdx, roundIndex) {
  ...
  try {
    ...
    myBuzzDocUnsub = onSnapshot(myBuzzDocRef, (snap) => {
      ...
      // ✅ Only count as "buzzed" if roundIndex matches current round
      const sameRound = data && Number(data.roundIndex ?? -1) === Number(roundIndex ?? -1);

      setBuzzerEnabled(!sameRound);
...
function attachBuzzListener(roomCode, frameIndex, roundIndex){
  if (buzzUnsub){ try{ buzzUnsub(); } catch(_){} buzzUnsub = null; }
  if (frameIndex == null) return;

  const colRef = frameBuzzesCol(roomCode, frameIndex);
  const qy = query(colRef, where("roundIndex","==", roundIndex), orderBy("ts","asc"));
...
// ðŸ”§ Helper: safely get the movie name for a given frame index
...
function startHostCorrectProcessor(roomCode, frameIdx, roundIndex /*, currentFrameGlobal */) {
  console.log("âœ… Optimized Host Correct Processor started for frame", frameIdx, "round", roundIndex);
...
async function processHostCorrectPending(roomCode) {
 ...
 for (const { uid, name, frame, roundIndex, ref } of toProcess) {
  ...
  batch.update(roomRef, {
    spectatorCorrectCount: increment(1),   // ðŸ‘ˆ keep per-frame tally accurate
  ...
  batch.delete(ref);
    ...
    await batch.commit();
    console.log(`âœ… Processed ${toProcess.length} spectator correct(s)`);
  } catch (e) {
    console.error("âŒ Host Correct Processor failed:", e);
  }
}
...
function subscribePlayerViewCorrectChatsForFrame(roomCode, frameIdx, roundIndex) {
  ...
  // ðŸ‘‡ match the screenshot wording
  textNode.textContent = '  guessed the movie correct';
...
function updatePlayerChatCorrectBadges() {
  try {
    const chatListEl = document.getElementById('chatList');
    if (!chatListEl) return;
    const badges = chatListEl.querySelectorAll('.spec-cc-badge');
    badges.forEach(badge => {
      const uid = badge.dataset.uid;
      if (!uid) return;
      const stat = spectatorStats.get(uid);
      const countSpan = badge.querySelector('span:last-child');
      if (stat && countSpan) countSpan.textContent = String(Number(stat.correctCount || 0));
    });
  } catch (e) {
    // non-fatal
  }
...
function renderPlayers(arr) {
  ...
  for (const p of arr) {
    ...
    // â­ referee star on avatar
const isRef = window.currentRoomData && window.currentRoomData.refereeUid === p.id;

// ---- Clean up any previous overlays (important on re-render) ----
logoEl.querySelectorAll('.ref-ring, .ref-badge').forEach(n => n.remove());
// reset any inline tweaks from older versions
logoEl.style.padding = '';
logoEl.style.boxShadow = '';
logoEl.style.border = '';
logoEl.style.overflow = 'hidden';
logoEl.style.position = 'relative'; // needed for absolute overlays

if (isRef) {
  // Make sure nothing clips the overlays
  li.style.overflow = 'visible';
  logoEl.style.overflow = 'visible';

  // Non-intrusive ring (does NOT change size of avatar)
  const ring = document.createElement('div');
  ring.className = 'ref-ring';
  Object.assign(ring.style, {
    position: 'absolute',
    inset: '-2px',
    border: '2px solid #ffd84d',
    borderRadius: '50%',
    boxShadow: '0 0 8px rgba(255,216,77,.7)',
    pointerEvents: 'none',
    zIndex: '2'
  });
  logoEl.appendChild(ring);

  // Tiny round REF badge
  ...
  // Non-ref rows must NOT clip future overlays
  li.style.overflow = 'visible';
}
    ...
    nameText.textContent = `${p.name || "Player"}${p.host ? " ðŸ‘‘" : ""}`;
    nameLine.appendChild(nameText);

    // ðŸŸ¡ this time we APPEND the referee label (not overwrite text)
    if (isRef) {
      const tag = document.createElement("span");
      tag.textContent = "Referee";
    ...
    if (iAmHost) {
      ...
      plus.className = "awardBtn";
      plus.textContent = "+10";
      plus.dataset.target = p.id;
      plus.onclick = () => adjustScore(p.id, 10);
      ...
      minus.onclick = () => adjustScore(p.id, -10);
  ...
  if (pcountEl) pcountEl.textContent = arr.length;
}
...
function showWinnerModalBoth(players, spectators) {
  ...
  title.textContent = 'ðŸ† Round Results';
  ...
  if (spectators && spectators.length) {
    ...
    spectators.slice(0, 10).forEach((s) => {
      ...
      countEl.style.padding = '6px 10px';
      countEl.style.borderRadius = '999px';
      countEl.textContent = `${Number(s.correctCount||0)} âœ”`;
...
// -------------------- Spectator Correct Counter (Live) --------------------
async function getTotalSpectatorCorrect(roomCode) {
  const snap = await getDocs(collection(db, "rooms", roomCode, "spectators"));
  let total = 0;
  snap.forEach(d => {
    const data = d.data() || {};
    total += Number(data.correctCount || 0);
  });
  return total;
}
    ...
    if (!lockBtn._bound) {
      ...
      lockBtn.addEventListener("click", async () => {
        ...
        try {
          ...
          showActionToast("Failed to toggle lock â€” check console");
        } finally {
...
function setBtnLoading(btn, loading, fallbackLabel='â­ Next') {
  if (!btn) return;
  ensureSpinnerStyles();
  if (loading) {
    ...
    btn.innerHTML = `<span class="spinner" aria-hidden="true"></span> Loadingâ€¦`;
  } else {
...
function updateLayoutForChatPanel() {
  ...
  // chat is â€œvisibleâ€ only if the element exists and isnâ€™t hidden by class or CSS
  const chatVisible =
    !!chatPanel &&
...
function renderSpectatorsListCard(items=[]) {
  ...
  items.forEach((s) => {
    ...
    cc.textContent = `${Number(s.correctCount || 0)} âœ”`;
...
function ensureRefereeBtnNearBuzzer() {
  ...
  refereeBtn.type = 'button';
  refereeBtn.textContent = 'Referee';
  refereeBtn.title = 'Enable automatic referee rotation';

  Object.assign(refereeBtn.style, {
    marginLeft: '8px',
    padding: '6px 10px',
    borderRadius: '8px',
    fontWeight: '900',
  ...
  refereeBtn.addEventListener('click', async () => {
    if (!iAmHost) return showActionToast("Only the host can choose referee");
    refereeBtn.disabled = true;
    try {
...
function showRefereeButtonHostOnly(show) {
  const btn = ensureRefereeBtnNearBuzzer();
...
buzzerBtn?.addEventListener("click", async ()=> {
  ...
  try {
    ...
    const rIdx = (window.currentRoomData?.roundIndex ?? 0);  // ðŸ‘ˆ current round
    await pressBuzzerOnce(room, idx, rIdx, myUid, name);
    if (buzzerStatus) buzzerStatus.textContent = "You buzzed!";
    ...
    if (String(e?.message || "").toLowerCase().includes("already buzzed")) {
      ...
      if (buzzerStatus) buzzerStatus.textContent = "Error â€” try again.";
...
onAuthStateChanged(auth, async (user) => {
  if (!user) return;
  if (!room) {
    showActionToast("Room missing â€” returning home");
  ...
  try {
    ...
    if (mode !== "spectator") {
      ...
      if (locked && !(roomData.hostUid === myUid)) {
        showActionToast("Room is locked â€” join as spectator", 2000);
  ...
  playersUnsub = onSnapshot(playersCol(room), async (snap) => {
   const list = [];
snap.forEach(d => list.push({ id: d.id, ...(d.data()||{}) }));
lastPlayersList = list;        // <-- store latest players
renderPlayers(list);
try { await assignNewHostIfNeeded(list); } catch(e){ console.warn(e); }
 ...
 // spectators collection (for counts + per-uid correctCount cache)
try {
  const spectatorsColRef = collection(db, "rooms", room, "spectators");
  spectatorsUnsub = onSnapshot(spectatorsColRef, (snap) => {
    ...
    // update stats map
    spectatorStats.clear();
    snap.forEach(d => {
      const data = d.data() || {};
      spectatorStats.set(d.id, {
        name: data.name || d.id,
        correctCount: Number(data.correctCount || 0)
      });
    });

    // refresh badges in the player chat if it's rendered
    try { updatePlayerChatCorrectBadges(); } catch(_) {}
...
roomUnsub = onSnapshot(roomDoc(room), { includeMetadataChanges: true }, (snap) => {
  ...
  // Re-render the players list whenever the referee changes so the tag updates live
const currentRefUid = d.refereeUid || "";
if (currentRefUid !== lastRefereeUid) {
  lastRefereeUid = currentRefUid;
  try { renderPlayers(lastPlayersList.slice()); } catch(_) {}
}
  ...
  const refereeOn = !!d.refereeAuto;
showRefereeButtonHostOnly(iAmHost && !endOfRound && !refereeOn);
...
if (mode !== "spectator" && !isSpectatorLocal) {
  ...
  // spectator chat is fully handled by spectatorChat.js â€“ ensure we don't keep a game.js listener around
  try { if (gameChatUnsub) gameChatUnsub(); } catch(_) {}
  gameChatUnsub = null;
}
  ...
  if (mode !== "spectator" && !isSpectatorLocal) {
attachBuzzListener(room, currentFrameIndex, roundIndex);
  } else {
    try { if (buzzUnsub) buzzUnsub(); } catch(_) {}
    if (buzzList) buzzList.innerHTML = '';
  }
...
// inside roomUnsub snapshot handler â€” after you computed idx and roundIndex and cf
if (iAmHost) {
...
export function cleanupGameListeners() {
  ...
  try { if (myBuzzDocUnsub) myBuzzDocUnsub(); } catch(_) {}
    ...
    try { if (specCorrectCountUnsub) specCorrectCountUnsub(); } catch(_) {}
...
(function checkMobileWarning() {
  function showMobileWarning() {
    ...
    msg.innerHTML = `
      âš ï¸ This website is <span style="color:#ff8e8e">not optimized</span> for mobile yet.<br>
...
(function(){
  ...
  function spotlightReloadBtn(){
    ...
    Spotlight.show(
      el,
      "If chat, buzz order, or anything looks off, tap â€œGlitch? Reloadâ€ to instantly resync the game.",
